...
#mycursor.execute(f"DROP TABLE IF EXISTS CATEGORY")
...
if reboot == "1":
    ...
    names_list, codes_list, description_list, store_list, product_show = L.Listing(product_nb, pages, name).listing()
    Sql(names_list, codes_list, description_list, store_list, product_show, name, category_name, category).drop_table(mycursor)
else :
    S.Sql(name, category_name, category, mydb, product_type).table_category(mycursor)
if reboot == "1":
    S.Sql(name, category_name, category, mydb, product_type).table_product(mycursor, names_list, codes_list, description_list, store_list, product_show)
else:
    selected1 = S.Sql(name, category_name, category, mydb, product_type).show_product(mycursor)
    selected2 = S.Sql(name, category_name, category, mydb, product_type).alt_product(mycursor, selected1)
    S.Sql(name, category_name, category, mydb, product_type).save_product(mycursor, selected2)
    S.Sql(name, category_name, category, mydb, product_type).display_saved(mycursor)